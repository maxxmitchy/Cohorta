...
import { ArrowLeft, Users, Activity, CheckCircle2, Circle, Radio, HelpCircle, Shield, Zap } from 'lucide-react';
...
export default function CommunityDetail() {
  ...
  const { communityDetailService } = useServices();
  const [community, setCommunity] = useState<CommunityDetailReadModel | null>(null);
  ...
  }, [communityId, communityDetailService]);
  ...
  // Determine primary display price for the big button
  // For a real app, this logic might exist in a Presenter/Selector
  const hasFree = community.pricing.some(p => p.type === 'free');
  const primaryPaidPlan = community.pricing.find(p => p.type === 'paid' && p.interval === 'month') 
                       || community.pricing.find(p => p.type === 'paid');

  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <Link to="/" className="mb-8 inline-flex items-center gap-2 text-sm font-medium text-neutral-500 hover:text-neutral-900">
        ...
        <div className="lg:col-span-8 flex flex-col gap-12">
          ...
          <section>
            ...
            <div className="relative border-l-2 border-neutral-100 pl-8 ml-3">
              {community.roadmap.map((item, index) => {
          ...
          <div className="sticky top-8 flex flex-col gap-6 rounded-2xl border border-neutral-200 bg-white p-6 shadow-xl shadow-neutral-200/50 sm:p-8">
            ...
            <div className="flex flex-col gap-4">
              ...
              {hasFree ? (
                ...
                <button className="flex h-14 w-full items-center justify-center rounded-xl bg-neutral-900 text-base font-bold text-white transition-transform hover:scale-[1.02] active:scale-[0.98]">
                  Join for {formatPricing(primaryPaidPlan)}
                </button>
              ) : (
            ...
            {!hasFree && community.pricing.length > 1 && (
              <div className="mt-2 text-center text-xs text-neutral-500">
                Also available: {community.pricing.filter(p => p !== primaryPaidPlan).map(p => formatPricing(p)).join(', ')}
              </div>
            )}
            
            {hasFree && community.pricing.some(p => p.type === 'paid') && (
              <div className="mt-2 text-center text-xs text-neutral-500">
